...
# TODO: get tradedates, zz500, target_stock, total_return, total_price
trade_dates = ...
zz500 = ...
...
class XSMOM:
    ...
    #                              direction='backward') 
            ...
            df_return = self.get_frame(data=total_return,
                                time_window=train_window, 
                                index_num = current_day_index,
                                direction='backward') 
    ...
    #         bottom = X[X < median].index
            ...
            # 获取用于计算未来收益率的数据框
            test_return = self.get_frame(data=total_return,
    ...
    def run(
        ...
        start_day = self.find_trade_day(trade_dates, start_day, 'forward')
        start_day_index = int(np.where(trade_dates == start_day)[0])
        end_day = self.find_trade_day(trade_dates, end_day, 'backward')
        ...
        if read is True and note is not None:
            ...
            strategy_return, strategy_signal = self.cal_strategy(period, step, start_day_index, skip_first_day,get_Xt) ###
